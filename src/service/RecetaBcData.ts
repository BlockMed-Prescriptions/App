...
export default class RecetaBcData {
  ...
  private static readonly USER_ROLE = "user_role";

  // TODO todos estos Subjects deben pasar a otra clase.
  private currentProfileSubject: Subject<Profile | null> =
    new Subject<Profile | null>();
  public currentProfile: Profile | null = null;
  public loading: Loading = { profile: false };
  private folderSuscription: Subject<RecetaFolder> =
    new Subject<RecetaFolder>();
  ...
  public async setCurrentProfile(profile: Profile | null) {
    ...
    if (profile) {
      await this.storeProfiles.setItem(
        RecetaBcData.CURRENT_PROFILE_DID,
        profile.didId
      );
    }
  }

  private changeLoading(key: "profile" | "role", value: boolean) {
    this.loading = { ...this.loading, [key]: value };
  }

  public async loadCurrentProfile() {
    this.changeLoading("profile", true);
    let didId = (await this.storeProfiles.getItem(
      RecetaBcData.CURRENT_PROFILE_DID
    )) as string | null;
    if (didId) {
    ...
    this.changeLoading("profile", false);
  ...
  public async deleteProfile(didId: string) {
    ...
    try {
      ...
      let index = profiles.findIndex(
        (profile: Profile) => profile.didId === didId
      );
  ...
  public async saveLastMessageDate(date: Date) {
    await this.ensureProfileData().setItem(
      this.buildKeyLastMessageDate(),
      date.toISOString()
    );
  }
  ...
  public async getLastMessageDate(): Promise<Date> {
    let date = (await this.ensureProfileData().getItem(
      this.buildKeyLastMessageDate()
    )) as string | null;
    if (!date) {
  ...
  public async getAllRecetas(): Promise<Receta[]> {
    ...
    for (let key of recetas) {
      if (key.startsWith(RecetaBcData.RECETAS_KEY)) {
        let receta = await this.getReceta(
          key.replace(RecetaBcData.RECETAS_KEY + ":", "")
        );
        result.push(receta);
      }
    }
  ...
  public async addTransaccionToReceta(
    receta: Receta,
    ...
    try {
      if (
        !receta.transacciones.find(
          (t) => t.hashTransaccion === transaccion.hashTransaccion
        )
      ) {
  ...
  private async safeRemoveRecetaFromFolder(
    receta: Receta,
    folder: RecetaFolder
  ) {
    let recetas = await this.getRecetasIdFromFolder(folder);
  ...
  public async moveRecetaToFolder(
    receta: Receta,
    from: RecetaFolder,
    to: RecetaFolder
  ) {
  ...
  public async getRecetasFromFolder(folder: RecetaFolder): Promise<Receta[]> {
    let recetas = await this.ensureProfileData().getItem(
      this.buildKeyFolder(folder)
    );
    if (!recetas) {
      return [];
  ...
  private async getRecetasIdFromFolder(
    folder: RecetaFolder
  ): Promise<string[]> {
  ...
  private async saveRecetasIdToFolder(
    folder: RecetaFolder,
    recetaIds: string[]
  ) {
    await this.ensureProfileData().setItem(
  ...
  private async saveProfiles(profiles: Profile[]) {
    ...
    await this.storeProfiles.setItem(
      RecetaBcData.PROFILES_KEY,
      JSON.stringify(profiles)
    );
  }

  public async saveUserRole(userRole: UserRole) {
    console.log("Guardar User mode", userRole);
    await this.storeProfiles.setItem(
      RecetaBcData.USER_ROLE,
      JSON.stringify(userRole)
    );
  }

  public async getUserRole() {
    let p: UserRole | null = await this.storeProfiles.getItem(
      RecetaBcData.USER_ROLE
    );
    if (!p) {
      return [];
  ...
  private buildKeyReceta(receta: Receta | string): string {
    ...
    else return this.buildKeyReceta(receta.id!);
  }
